...
# Retrieve the OpenRouteService API token from environment variables
TOKEN = os.getenv('OPEN_SERVICE_API')
client = openrouteservice.Client(key=TOKEN)

@app.route('/')
def index():
    # Initialize an empty map with a search box and empty directions box
    start_coords = (40.11611304201861, -75.11012558782619)
    folium_map = folium.Map(location=start_coords, zoom_start=15)
    return render_template('index.html', map_html=folium_map._repr_html_(), directions=[])

@app.route('/submit_coordinates', methods=['POST'])
def submit_coordinates():
    # Extract the latitude and longitude from the submitted form data
    ...
    start_coords = (float(latitude), float(longitude))

    # Create a folium map centered at the input coordinates
    folium_map = folium.Map(location=start_coords, zoom_start=15)
